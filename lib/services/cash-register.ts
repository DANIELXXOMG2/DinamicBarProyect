...
export class CashRegisterService {
  ...
  static async getCurrentCashRegister(): Promise<CashRegisterWithTransactions | null> {
    try {
  ...
  static async openCashRegister(data: OpenCashRegisterData): Promise<CashRegister> {
    try {
      // Verificar que no haya una caja abierta
      const currentRegister = await this.getCurrentCashRegister()
      if (currentRegister) {
        throw new Error('Ya hay una caja registradora abierta. Debe cerrarla primero.')
      }
  ...
  static async closeCashRegister(data: CloseCashRegisterData): Promise<CashRegisterSummary> {
    try {
      const currentRegister = await this.getCurrentCashRegister()
      if (!currentRegister) {
  ...
  static async addTransaction(data: CreateTransactionData): Promise<CashTransaction> {
    try {
      const currentRegister = await this.getCurrentCashRegister()
      if (!currentRegister) {
        throw new Error('No hay una caja registradora abierta')
      ...
      const newTotalCash = data.type === TransactionType.INCOME 
  ...
  static async getCashRegisterSummary(registerId: string): Promise<CashRegisterSummary> {
    try {
      ...
      const expectedCash = register.openingAmount + register.totalSales + totalIncome - totalExpenses
      ...
      return {
        ...
        totalExpenses,
        expectedCash,
        difference: register.closingAmount ? register.closingAmount - expectedCash : 0
      }
    } catch (error) {
  ...
  static async getCashRegisterHistory(limit: number = 10): Promise<CashRegister[]> {
    try {
  ...
  static async getCashRegisterTransactions(registerId: string): Promise<CashTransaction[]> {
    try {
      ...
      throw new Error('Error al obtener las transacciones de la caja registradora')
    }
  }

  // Verificar si hay una caja abierta
  static async isCashRegisterOpen(): Promise<boolean> {
    try {
      const currentRegister = await this.getCurrentCashRegister()

...
import { removeItemFromTab, updateTabItem } from '@/lib/services/tabs';
...
export async function PUT(
  ...
  try {
    ...
    const tab = await updateTabItem(id, productId, { quantity });
...
export async function DELETE(
  ...
  try {
    const { id, productId } = await params;
    const tab = await removeItemFromTab(id, productId);

...
export async function generateProducts(
  ...
  for (const category of categories) {
    ...
    // Encuentra la entrada del catálogo que coincide con el nombre de la categoría
    const catalogEntry = Object.entries(productCatalog).find(
      ([name]) => name === category.name
    );

    // Si se encuentra una entrada
    if (catalogEntry) {
